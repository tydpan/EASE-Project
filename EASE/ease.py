...
def possible_type(avg_cap_list):
    ...
    input = a list includes average capacity of different types of resources
    output = by filter out impposible resources, the outcome will be the possible type of resources used to generate electricity
    """
    path = os.path.dirname(__file__)
    path = os.path.join(path, 'Arranged_Data', 'average_plant_capacity.csv')
...
def rf_fluctuation(prec, ts, tw, ws):
    """
    This function is used to evaluate standard deviation of maximun wt% based on state vote 
    creat a null list to store state vote (keys) have maximun wt%
...
def rev_plot(avg_cost, capacity, e_type, label, avg_cost_conv=0, capacity_conv=0):
    """
    The purpose of this function is to make plot for clean, conventional, and total revenue 
    use average cost, capacity, resource type as input
...
def avg_cost(vote):
    """
    This function is used to evaluate average cost of each type of resource
...
def clean_or_conv(possible_type_list):
    """
    The purpose of this function is to distinguish whether the possible type resources are belong to clean list or conventional list
...
def sort_and_pick(source_list):
    """
    This function is to sort and pick the type of resources
    When the difference of p-value less than 0.05, pick the one which has larger capacity
    return resoruce type
    """
    source_list.sort()
...
def suggest(prec, ts, tw, ws, capacity):
    """
    This is a wrapping function combined previous functiuons used to plot revenue and show carbon dixoide emission
    ...
    evaluate and compare carbon dioxide emission from only use conventional resources and use clean,conventional combined resources
    return clean revenue, conventional revenue, total revenue, carbonx dioxide emission plot
    """
